...
router.patch('/trust/:userId', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (reason) {
      user.notifications.push({ message: `⚙️ Admin updated your trust score to ${trustScore}. Reason: ${reason}`, type: 'info' });
      await user.save();
    }
    res.json(user);
  } catch (err) {
...
router.post('/warn/:userId', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.params.userId);
    user.notifications.push({ message: `⚠️ Warning from admin: ${message}`, type: 'warning' });
    await user.save();
...
router.post('/refund/:sessionId', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (session.status === 'completed') {
      ...
      mentor.notifications.push({ message: `↩️ Admin issued refund for session (${session.skill}). ${session.tokenCost} tokens deducted.`, type: 'warning' });
    } else if (session.status === 'pending' || session.status === 'active') {
      learner.walletBalance += session.tokenCost;
      learner.escrowBalance -= session.tokenCost;
    }
...
router.post('/resolve-dispute/:sessionId', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (refund) {
      // Refund learner — deduct from mentor
      const learner = await User.findById(session.learner);
      const mentor = await User.findById(session.mentor);
      learner.walletBalance += session.tokenCost;
      mentor.walletBalance = Math.max(0, mentor.walletBalance - session.tokenCost);
      ...
      mentor.notifications.push({ message: `⚠️ Dispute resolved against you. Tokens refunded to learner. Trust score -10.`, type: 'error' });
      await learner.save();
      await mentor.save();
      session.status = 'refunded';
    } else {
      // Dispute rejected — session stays completed
      const learner = await User.findById(session.learner);
      learner.notifications.push({ message: `ℹ️ Your dispute was reviewed. Outcome: ${resolution}`, type: 'info' });
      await learner.save();
